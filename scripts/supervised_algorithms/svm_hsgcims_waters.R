...
run_experiment <- function(method, imbal_train, imbal_test, output_prefix) {
    ...
    if (method == "none") {
        ...
        boruta_out <- Boruta(Group ~ .,
            data = imbal_train, doTrace = 2,
            maxRuns = 500, pValue = 0.01, mcAdj = TRUE
        )
        ...
        ga_fitness <- function(binary_vector) {
            ...
            set.seed(7234)
            mod <- tryCatch(
                train(Group ~ ., data = data_sel, method = "svmLinear", trControl = ctrl, tuneGrid = grid),
                error = function(e) {
                    return(NULL)
                }
            )
    ...
    grid_radial <- expand.grid(C = 2^(seq(-5, 5, 1)))
    start_time <- Sys.time()
    svm_model <- train(Group ~ .,
        data = smote_train, method = "svmLinear",
        trControl = trctrl, tuneGrid = grid_radial, metric = "Accuracy"
    )
    training_time <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))

    # Create training performance plot
    training_plot <- ggplot(svm_model$results, aes(x = factor(C), y = Accuracy)) +
        geom_point(color = viridis(11)[5], size = 3) +
        geom_line(group = 1, color = viridis(11)[5]) +
        labs(
            title = paste("Training Performance -", toupper(method)),
            x = "Cost (C) [log2 scale]", y = "Cross-Validated Accuracy"
        ) +
        theme_minimal() +
        theme(plot.title = element_text(hjust = 0.5, face = "bold"))
    ggsave(
        filename = file.path(output_dir, paste0("svm_training_plot_", output_prefix, ".png")),
        plot = training_plot, width = 10, height = 6, dpi = 300
    )

    # Final model training and evaluation
    final_svm <- train(Group ~ .,
        data = smote_train, method = "svmLinear",
        tuneGrid = svm_model$bestTune
    )
    support_vectors <- sum(final_svm$finalModel@nSV)
    best_C <- svm_model$bestTune$C
    best_cv_accuracy <- max(svm_model$results$Accuracy)
    best_cv_kappa <- max(svm_model$results$Kappa)
    pred_test <- predict(final_svm, newdata = test_processed)
    ...
    ggsave(
        filename = file.path(output_dir, paste0("svm_confusion_plot_", output_prefix, ".png")),
...
summary_df <- do.call(rbind, lapply(results_list, function(x) {
    data.frame(
        ...
        Support_Vectors = x$support_vectors,
        Test_Accuracy = round(as.numeric(x$test_accuracy), 4),
        Test_Kappa = round(as.numeric(x$test_kappa), 4)
    )
}))
print(summary_df)
write.csv(summary_df, file = file.path("data", "summary_results_svm.csv"), row.names = FALSE)
...
ggsave(
    filename = file.path(output_dir, "combined_training_svm_plots.png"),
...
ggsave(
    filename = file.path(output_dir, "combined_confusion_svm_plots.png"),
